    ...
    private static String getChar(int x, int y){
        ...
        for (Character character: main.allteams) {
            if (character.getCoords()[0] == x && character.getCoords()[1] == y){
                ...
                if (main.listdark.contains(character)) out = "|" + (AnsiColors.ANSI_GREEN + character.getInfo().charAt(0) + AnsiColors.ANSI_RESET);
                if (main.listlight.contains(character)) out = "|" + (AnsiColors.ANSI_BLUE + character.getInfo().charAt(0) + AnsiColors.ANSI_RESET);
                break;
            }
        }
        return out;
